...
Each component has full access to the outside network (network type `bridge`) unless changed to `none` in the 
...
The following environment variables are injected into the container:
 ...
 - `KBC_CONFIGVERSION`: Version of the configuration, or empty if the configuration is not named (`configData` was used in the [API call](https://kebooladocker.docs.apiary.io/#reference/run/create-a-job/run-job)).
 ...
 - `KBC_STAGING_FILE_PROVIDER`: Either `aws` or `azure`, depending on which kind of [Stack](/overview/api/#regions-and-endpoints) the container is running. The value refers to the file storage used during [file import and export operations](https://developers.keboola.com/integrate/storage/api/import-export/).
 - `KBC_PROJECT_FEATURE_GATES`: Comma-separated list of feature gates activated for the current project. Feature gates are considered internal, and they may disappear without notice. We recommend you check with our support before relying on any feature gates.rec
 - `KBC_COMPONENT_RUN_MODE`: Either `run` or `debug`. The value `debug` is used in case debug mode was used to run the job
 https://developers.keboola.com/extend/component/running/#debugging. For example, this environment variable can help switch the component to a more verbose logging.
 - `KBC_DATA_TYPE_SUPPORT`: Either `authoritative`, `hints`, or `none`. The value `authoritative` means that the component generates columns with data types in the schema node. The value `hints` means the component generates columns without data types in the schema node. The value `none` means that the component generates the names of columns in the columns node. 
...
the contents of STDOUT and [STDERR](https://en.wikipedia.org/wiki/Standard_streams#Standard_error_.28stderr.29)
...
It is possible to modify the above behavior so that all errors are User Errors regardless of the exit code.
This is done by setting `no_application_errors` in the [component configuration](https://components.keboola.com/).
