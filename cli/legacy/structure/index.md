...
permalink: /cli/legacy/structure/
redirect_from:
  - /cli/structure/
---
...
The initial configuration of your local directory can be done using the [init command](/cli/legacy/commands/sync/init/). This command initializes 
...
you want to work with locally during the [init](/cli/legacy/commands/sync/init/) command. Alternatively, you can ignore the development branches concept and work exclusively
...
{: .image-popup}
![Screenshot -- A configuration directory example](/cli/legacy/structure/directory-example.jpg)
...
in the [manifest](/cli/legacy/structure/#manifest). After copying, run the [persist command](/cli/legacy/commands/local/persist/) to generate a new ID for the configuration and update it in the manifest.
...
{: .image-popup}
![Screenshot -- A configuration rows directory example](/cli/legacy/structure/directory-rows-example.jpg)
...
{: .image-popup}
![Screenshot -- A transformation directory example](/cli/legacy/structure/directory-transformation-example.jpg)
...
{: .image-popup}
![Screenshot -- Variables in the UI](/cli/legacy/structure/variables-ui.jpg)

When you [pull](/cli/legacy/commands/sync/pull/) them to the local directory, the structure will look like this:

{: .image-popup}
![Screenshot -- Configuration directory with the variables](/cli/legacy/structure/variables-directory.jpg)
...
{: .image-popup}
![Screenshot -- Shared code directory](/cli/legacy/structure/shared-code-ui.jpg)

It will move to the `_shared` directory:

{: .image-popup}
![Screenshot -- Shared code directory](/cli/legacy/structure/shared-code-directory.jpg)

The code in the transformation file `blocks/block-1/join/code.sql` will then be replaced with:

{: .image-popup}
![Screenshot -- Shared code code](/cli/legacy/structure/shared-code-code.jpg)
...
{: .image-popup}
![Screenshot -- Scheduler directory](/cli/legacy/structure/scheduler-directory.jpg)
...
{: .image-popup}
![Screenshot -- An orchestration directory](/cli/legacy/structure/directory-orchestration-example.png)
...
- `allowTargetEnv`: Boolean (default: `false`)
  ...
  - For more information, see the [--allow-target-env](/cli/legacy/commands/sync/init/#options) option in the [kbc sync init](/cli/legacy/commands/sync/init/) command.
- `sortBy`: Property name used for sorting configurations (default: `id`)
- `naming`: Rules for directory naming ([see details](/cli/legacy/structure/#naming))
...
Directory names for configurations follow the rules in the [manifest](/cli/legacy/structure/#manifest) under the `naming` section.  
...
Use the [fix-paths](/cli/legacy/commands/local/fix-paths/) command to rebuild the directory structure with updated naming rules.
